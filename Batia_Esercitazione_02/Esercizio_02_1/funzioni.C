...
void scriviSuFile(const string& nomeFile, const vector<double>& dati1, const vector<double>& dati2, const vector<double>& dati3, const vector<double>& dati4) {
    ...
    for (size_t i = 0; i < dati1.size(); ++i) {
        fileOutput << dati1[i] << "\t" << dati2[i] << "\t" << dati3[i] << "\t" << dati4[i] << std::endl;
    }
    fileOutput.close();
