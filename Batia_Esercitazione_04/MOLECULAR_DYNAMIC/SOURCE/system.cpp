...
void System :: read_configuration(string modello_output){
  ...
  if(_restart){
    if(_sim_type > 1){
      ...
      cinf.open("../OUTPUT/CONFIG/TERM"+modello_output+"/config.xyz"); //+modello_output+
    }
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & tconfig.xyz not match!" << endl;
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
