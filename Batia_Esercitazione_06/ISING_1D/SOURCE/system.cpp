...
void System :: initialize(string modello_input , string modello_output){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream couta("../OUTPUT/"+modello_output+"/acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/" + modello_input); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open("../OUTPUT/" + modello_output + "/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
...
void System :: initialize_velocities(string modello_output){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open("../OUTPUT/CONFIG/TERM"+modello_output+"/velocities.out");  //"../INPUT/CONFIG/" + modello_output + "/velocities.in"
    if(cinf.is_open()){                                                
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/" + modello_output + "/potential_energy.dat");
        ...
        ofstream coutpr("../OUTPUT/" + modello_output + "/pressure.dat");
        ...
        ofstream coutgr("../OUTPUT/" + modello_output + "/gofr.dat");
...
//cinf.open("../OUTPUT/CONFIG/TERM"+modello_output+"/config.xyz");
...
void System :: averages(int blk, string modello_output){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/" + modello_output + "/potential_energy.dat",ios::app);
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure){
    coutf.open("../OUTPUT/" + modello_output + "/pressure.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT/" + modello_output + "/acceptance.dat",ios::app);
