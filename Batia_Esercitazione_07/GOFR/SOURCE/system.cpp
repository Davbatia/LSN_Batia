  ...
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if(_sim_type == 1){       // LJ system
      ...
      for(int j=0; j<_ndim; j++){
        shift(j) = _rnd.Rannyu(-1.0,1.0) * _delta; // uniform distribution in [-_delta;_delta), da fixare per avere 50% acceptance
      }
...
void System :: initialize(string modello_input , string modello_output){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ofstream couta("../OUTPUT/Esercizio_07_04/"+modello_output+"/acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input("../INPUT/" + modello_input); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    input >> property;
  ...
  input.close();
  this->read_configuration(modello_output);
  this->initialize_velocities( modello_output);
  coutf << "System initialized!" << endl;
  coutf.close();
  return;
}

void System :: initialize_velocities(string modello_output){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/NVE/TERM"+modello_output+"/velocities.out");  //"../INPUT/CONFIG/" + modello_output + "/velocities.in"
    if(cinf.is_open()){                                                
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
...
void System :: initialize_properties(string modello_output){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/Esercizio_07_04/" + modello_output + "/potential_energy.dat");
        ...
        ofstream coutk("../OUTPUT/" + modello_output + "/kinetic_energy.dat");
        ...
        ofstream coutt("../OUTPUT/" + modello_output + "/total_energy.dat");
        ...
        ofstream coutte("../OUTPUT/" + modello_output + "/temperature.dat");
        ...
        ofstream coutpr("../OUTPUT/Esercizio_07_04/" + modello_output + "/pressure.dat");
        ...
        ofstream coutgr("../OUTPUT/Esercizio_07_04/" + modello_output + "/gofr.dat");
        ...
        ofstream coutpr("../OUTPUT/" + modello_output + "/magnetization.dat");
        ...
        ofstream coutpr("../OUTPUT/" + modello_output + "/specific_heat.dat");
        ...
        ofstream coutpr("../OUTPUT/" + modello_output + "/susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open("../OUTPUT/" + modello_output + "/output.dat",ios::app);
...
void System :: finalize(string modello_output){
  this->write_configuration(modello_output);
  _rnd.SaveSeed();
  ofstream coutf;
  coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/output.dat",ios::app);
...
void System :: write_configuration(string modello_output){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open("../OUTPUT/CONFIG/" + modello_output + "/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    this->write_velocities(modello_output);
  } else {
    coutf.open("../OUTPUT/CONFIG/"+modello_output+"/config.spin");
...
void System :: write_XYZ(int nconf, string modello_output){
  ofstream coutf;
  coutf.open("../OUTPUT/CONFIG/" + modello_output + "/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(string modello_output){
  ofstream coutf;
  coutf.open("../OUTPUT/CONFIG/" + modello_output + "/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(string modello_output){
  ifstream cinf;
  if(_restart){
    if(_sim_type > 1){
      int spin;
      if(_sim_type == 2){
      cinf.open("../OUTPUT/CONFIG/TermMetropolis/config.spin");
      }else{
        cinf.open("../OUTPUT/CONFIG/TermGibbs/config.spin"); 
      }
      for(int i=0; i<_npart; i++){
        cinf >> spin;
      ...
      cinf.open("../INPUT/CONFIG/NVE/TERM"+modello_output+"/config.xyz");
    }
    if(cinf.is_open()){
    string comment;
    string particle;
...
void System :: block_reset(int blk, string modello_output){ // Reset block accumulators to zero
  ofstream coutf;
  if(blk>0){
    coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/output.dat",ios::app);
...
void System :: averages(int blk, string modello_output){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT/" + modello_output + "/kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT/" + modello_output + "/total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/" + modello_output + "/temperature.dat",ios::app);
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure){
    coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/pressure.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output +"/gofr.dat",ios::app);
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    coutf.open("../OUTPUT/"+modello_output+"/magnetization.dat",ios::app);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    coutf.open("../OUTPUT/"+modello_output+"/specific_heat.dat",ios::app);
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/"+modello_output+"/susceptibility.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open("../OUTPUT/Esercizio_07_04/" + modello_output + "/acceptance.dat",ios::app);
...
void System :: print_istant(string modello_output){

  ofstream coutre;
  if (_measure_temp){
    coutre.open("../OUTPUT/"+modello_output+"/istant_temp.out",ios::app);
