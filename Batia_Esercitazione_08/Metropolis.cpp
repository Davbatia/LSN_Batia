...
void Metropolis :: sa_step(double (*fun)(vec , vec), double (*fun2)(vec ,vec )){
    ...
    delta = static_cast<double>(1./sqrt(_step_index_sa+1));
...
void Metropolis :: print_results(vec ave){
    ...
    for (int i=1; i<_nblocks; i++){
        ...
        if (_sa == false) { WriteResults << setw(7) << i << setw(15) << prog_sum[i] << setw(15) << error_value(prog_sum[i], prog_sum2[i],i) << setw(15) << prog_acc[i] << endl; }

        if (i==_nblocks-1){
            final_ave = prog_sum[i];
            final_sigma = error_value(prog_sum[i], prog_sum2[i],i);
