...
Random :: Random(){int seed[4];    // array of dimension four containing the four seeds for the random number generator
...
// Nel file originale il codice tra {} era scritto nel main
...
double Random :: UnifTheta(void) {
  // Method to generate an agle beetween 0 and Pi without knowing Pi
  double x,y;
  do{
     x=Rannyu(-1 , 1);
     y=Rannyu(0,1);
...
double Random :: Exponential(double lambda) {
   ...
   double y=Rannyu();
   return -1.0/lambda * log(1-y);
}

double Random :: Lorentz(double gamma) {
   // This function generates a random number from a Lorentz distribution with given gamma (assuming mu = 0)
   double y=Rannyu();
